...
async function inipesan() {
  const { value: inputNama } = await swals.fire({
    title: 'Masukin Nama Kamu',
    text: 'Ketik nama panggilanmu ya!',
    input: 'text',
    inputPlaceholder: 'Contoh: Vira',
    confirmButtonText: '✨ Lanjut',
    showCancelButton: false,
  });
  
  if (inputNama && inputNama.length < 11 && inputNama.trim() !== "") {
    nama = inputNama.trim();
    vketikhalo = "Hai, " + nama + " ✨";
    mulainama();
  } else {
...
function ftmuncul() {
  ...
  const stikerList = [
    deffotostiker,
    document.getElementById('fotostiker1').src,
    document.getElementById('fotostiker2').src,
    document.getElementById('fotostiker3').src,
    document.getElementById('fotostiker4').src,
    document.getElementById('fotostiker5').src
  ];
...
function aktipesan7() {
  ...
  // Tampilkan pesan 7 langsung
  const pesan7 = document.getElementById('pesan7');
  pesan7.style.display = "block";
  pesan7.style.opacity = "1";
  pesan7.style.transform = "scale(1)";
